...
class FiniteElementSolver : public solver::Solver, public solver::IFunction, public solver::BoundaryFunction {
   private:
    ...
    const std::vector<std::shared_ptr<domain::FieldFunction>> auxiliaryFieldsUpdaters;

   public:
    FiniteElementSolver(std::string solverId, std::shared_ptr<domain::Region> region, std::shared_ptr<parameters::Parameters> options,
                        std::vector<std::shared_ptr<boundaryConditions::BoundaryCondition>> boundaryConditions, std::vector<std::shared_ptr<domain::FieldFunction>> auxiliaryFields);
