...
class EssentialGhost : public Ghost {
   private:
    ...
    const std::shared_ptr<ablate::domain::ExactFunction> boundaryFunction;
   ...
   public:
    EssentialGhost(std::string boundaryName, std::vector<int> labelId, std::shared_ptr<domain::ExactFunction> boundaryFunction, std::string labelName = {}, bool enforceAtFace = false);
};
}  // namespace ablate::finiteVolume::boundaryConditions
