...
void testingResources::MpiTestFixture::RunWithMPI() const {
    ...
    std::stringstream mpiFullCommand;
    mpiFullCommand << MpiTestFixture::mpiCommand << " ";
    mpiFullCommand << "-n " << mpiTestParameter.nproc << " ";
    mpiFullCommand << "\"" << ExecutablePath() << "\" ";
    mpiFullCommand << InTestRunFlag << " ";
    mpiFullCommand << "--gtest_filter=" << TestName() << " ";
    mpiFullCommand << mpiTestParameter.arguments << " ";
    mpiFullCommand << " > " << OutputFile() << " 2>&1";

    auto exitCode = std::system(mpiFullCommand.str().c_str());
    if (exitCode != 0) {
